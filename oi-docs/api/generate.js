...
function buildPO(d) {
  ...
  const tRows=[['Sub-Total',d.subtotal,false],['GST (10%)',d.gst,false],['TOTAL (incl. GST)',d.total,true]].map(([lbl,val,big])=>new TableRow({children:[
...
function buildInvoice(d) {
  ...
  const tRows=[['Sub-Total',d.subtotal,false],['GST',d.gst,false],['TOTAL',d.total,true]].map(([lbl,val,big])=>new TableRow({children:[

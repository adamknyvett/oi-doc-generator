...
const NAVY = "0A1628", TEAL = "00B4D8", ACCENT = "0077B6",
      LTGRAY = "F4F7FA", MID = "8CA0B5", WHITE = "FFFFFF";

function money(v) {
  return "$" + parseFloat(v || 0).toLocaleString("en-AU", {
...
const teal = { style: BorderStyle.SINGLE, size: 8, color: TEAL };
...
function tcBorder(text, { bold=false, bg="FFFFFF", w=4680, align="left", color="111111", sz=18 }={}) {
  return new TableCell({
    width: { size: w, type: WidthType.DXA },
    shading: { fill: bg, type: ShadingType.CLEAR },
...
function buildDoc(d, docType) {
  ...
  const ref3val   = isInv ? (d.due || "30 days") : (d.delivery || "As per quote");
  ...
  // Parties
  const fromAddrStr = fromAddr.replace(/\\n/g, " | ");
  const toAddrStr   = toAddr.replace(/\\n/g, " | ");
  children.push(new Table({
    ...
    rows: [
      ...
      new TableRow({ children: [tc(fromAddrStr, { w:4560, color:"444444", sz:16 }), tc(toAddrStr + (toContact ? " | Attn: " + toContact : ""), { w:4800, color:"444444", sz:16 })] }),
  ...
  // Description
  if (d.description) {
    children.push(sectionLabel("Project Description"));
    children.push(new Paragraph({
      spacing: { before: 0, after: 160 },
      children: [new TextRun({ text: d.description, font: "Calibri", size: 17, color: "333333" })],
    }));
  }

  // Line items
  children.push(sectionLabel(isInv ? "Services Rendered" : "Line Items"));
  ...
  children.push(new Table({
    ...
    rows: [
      new TableRow({ children: [
        ...
        tcBorder("Sub-Total (excl. GST)", { w:2700, color:"555555", sz:17 }),
        tcBorder(money(d.subtotal),       { w:1620, align:"right", sz:17 }),
      ]}),
      new TableRow({ children: [
        new TableCell({ width:{size:5040,type:WidthType.DXA}, borders:{top:none,bottom:btm,left:none,right:none}, margins:{top:60,bottom:60,left:0,right:0}, children:[new Paragraph({children:[]})] }),
        tcBorder("GST (10%)",     { w:2700, color:"555555", sz:17 }),
        tcBorder(money(d.gst),    { w:1620, align:"right", sz:17 }),
      ]}),
      new TableRow({ children: [
        ...
        new TableCell({ width:{size:2700,type:WidthType.DXA}, borders:{top:teal,bottom:none,left:none,right:none}, margins:{top:80,bottom:80,left:120,right:120},
        ...
        new TableCell({ width:{size:1620,type:WidthType.DXA}, borders:{top:teal,bottom:none,left:none,right:none}, margins:{top:80,bottom:80,left:120,right:120},
  ...
  if (d.bank_bsb || d.bank_acct || d.bank_name) {
    children.push(sectionLabel("Payment Details"));
    const bankRows = [
      ["Account Name",   d.bank_name],
      ["BSB",            d.bank_bsb],
      ["Account Number", d.bank_acct],
      ["Bank",           d.bank_branch],
    ].filter(r => r[1]);
    children.push(new Table({
      width: { size: 9360, type: WidthType.DXA },
  ...
  // Notes
  if (d.notes) {
    children.push(sectionLabel("Notes"));
    children.push(new Paragraph({
      spacing: { before: 0, after: 160 },
      children: [new TextRun({ text: d.notes, font: "Calibri", size: 16, color: "444444" })],
    }));
  }
  ...
  if (!isInv) {
    ...
    children.push(sectionLabel("Authorisation"));
    children.push(new Table({
      width: { size: 9360, type: WidthType.DXA },
